...
export default function Admin() {
  ...
  const toggleItem = (type, id, action) => {
    if (window.confirm(`${action === "desactiver" ? "Désactiver" : "Réactiver"} cet ${type} ?`)) {
                  ...
                  {data.map(u => (
                    <tr key={u.id}>
                      ...
                      <td className="actions">
                        <button onClick={() => navigate(`/admin/utilisateurs/modifier/${u.id}`)}>
                          Modifier
                        </button>
                        <button onClick={() => {
                ...
                <tbody>
                  {data.map(ev => (
                    <tr key={ev.id}>
                      ...
                      <td className="actions">
                        <button onClick={() => navigate(`/admin/evenements/modifier/${ev.id}`)}>
                          Modifier
                        </button>
                        {ev.actif
                          ? <button onClick={() => toggleItem("evenements", ev.id, "desactiver")}>Désactiver</button>
                          : <button onClick={() => toggleItem("evenements", ev.id, "reactiver")}>Réactiver</button>
                        }
                      </td>
                    </tr>
                  ))}
                ...
                <tbody>
                  {data.map(r => (
                    <tr key={r.id}>
                      ...
                      <td className="actions">
                        <button onClick={() => navigate(`/admin/reservations/modifier/${r.id}`)}>
                          Modifier
                        </button>
                        {r.actif
                          ? <button onClick={() => toggleItem("reservations", r.id, "desactiver")}>Désactiver</button>
                          : <button onClick={() => toggleItem("reservations", r.id, "reactiver")}>Réactiver</button>
                        }
                      </td>
                    </tr>
                  ))}
